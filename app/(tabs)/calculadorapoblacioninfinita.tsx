...
export default function calculadorapoblacioninfinita(): JSX.Element {
  ...
  // Función para calcular el tamaño de la muestra (población infinita)
  const calcularMuestra = () => {
    ...
    if (isNaN(z) || isNaN(e) || e <= 0 || e >= 1) {
      setSampleSize(null);
      return;
    }

    // Fórmula de tamaño de muestra para población infinita
    const n = (Math.pow(z, 2) * p * (1-p)) / Math.pow(e, 2);
    setSampleSize(Math.ceil(n));

    // Fórmula lineal
    setFormula(`n = (Z² * p * (1-p)) / E²`);
    setSubstitution(
      `n = (${z}² * ${p} * (1-${p})) / (${e})²`
    );
  ...
  useEffect(() => {
    calcularMuestra();
  }, [marginError, confidenceLevel]);

  return (
    <ScrollView contentContainerStyle={estilos.container}>
      <Text style={estilos.titulo}>Calculadora de Población de Muestra Infinita</Text>
